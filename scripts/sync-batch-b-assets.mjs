...
const assetPattern = /\/voyah-resources\/images\/car\/(?:titan|titan_blackedition|titan_X8|car_logo\/titan|car_logo\/titan_blackedition|car_logo\/titan_X8)[^'"\s)]+/g
  ...
  new Promise((resolve) => {
    ...
    https
      .get(url, (response) => {
        ...
        response.pipe(file)
        file.on('finish', () => {
          file.close(() => resolve({ assetPath, ok: true, skipped: false }))
        })
      })
      .on('error', () => {
        resolve({ assetPath, ok: false, code: 'ERR' })
      })
  })
